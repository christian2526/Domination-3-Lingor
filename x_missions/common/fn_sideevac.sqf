...
__TRACE_1("","_owngroup")
private _pilot1 = _owngroup createUnit [d_sm_pilottype, _poss, [], 60, "FORM"];
__TRACE_1("","_pilot1")
_pilot1 call d_fnc_removenvgoggles_fak;

private _pilot2 = _owngroup createUnit [d_sm_pilottype, getPosATL _pilot1, [], 0, "FORM"];
__TRACE_1("","_pilot2")
	...
	if (!alive _pilot1 && {!alive _pilot2}) then {
		...
		if (!_rescued) then {
			...
			if (alive _pilot1) then {
				...
				private _nobjs = _pilot1 nearEntities ["CAManBase", 20];
				__TRACE_1("","_nobjs")
			...
			if (!_rescued) then {
				if (alive _pilot2) then {
					...
					private _nobjs = _pilot2 nearEntities ["CAManBase", 20];
					__TRACE_1("","_nobjs")
					if !(_nobjs isEqualTo []) then {
						{
							if (alive _x && {isPlayer _x && {!(_x getVariable ["xr_pluncon", false]) && {!(_x getVariable ["ace_isunconscious", false]) && {_x call _aiver_check_fnc}}}}) exitWith {
