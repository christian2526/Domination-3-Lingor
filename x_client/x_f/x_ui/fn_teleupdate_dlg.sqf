	...
	if (_lbdata != "D_BASE_D") then {
		if (_lbdata != "D_SQL_D") then {
			...
			if (d_respawnatsql == 0 && {_lbdata == "D_SQL_D" && {!(player getVariable ["xr_isleader", false]) && {count units (group player) > 1}}}) then {
				...
				private _lbcolor = if (xr_respawn_available && {alive _leader} && {!(_leader getVariable ["xr_pluncon", false])} && {!(_leader getVariable ["ace_isunconscious", false])} && {_emptycargo > 0 || {(getPos _leader) select 2 < 10}} && {!underwater _leader}) then {
