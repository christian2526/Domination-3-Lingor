...
#else
if (d_with_ranked && {player inArea (d_base_array select 0) || {player inArea (d_base_array select 1)}} && {d_last_base_repair != -1}) exitWith {
#endif
...
#else
if (d_with_ranked && {player inArea (d_base_array select 0) || {player inArea (d_base_array select 1)}}) then {d_last_base_repair = time + 300};
#endif
