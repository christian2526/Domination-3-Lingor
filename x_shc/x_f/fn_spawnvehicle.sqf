...
if (_sim in ["AIRPLANE", "HELICOPTER", "AIRPLANEX", "HELICOPTERX", "HELICOPTERRTD"]) then {
	...
	_veh = createVehicle [_typev1, _posv1, [], 0, "FORM"];
	...
	_veh setDir _azi;
	_veh setVehiclePosition [_veh, [], 0, "FORM"];
};
...
if (_addkills) then {
	...
	if !(_veh isKindOf "Air") then {
		_veh addEventHandler ["Killed", {[[20, 3, 2, 1], _this select 1, _this select 0] remoteExecCall ["d_fnc_AddKills", 2]}];
	} else {
		_veh addEventHandler ["Killed", {[[30, 3, 2, 1], _this select 1, _this select 0] remoteExecCall ["d_fnc_AddKills", 2]}];
	};
#endif
